...
def select_02():
    ...
    conn = conectar()
    cursor = conn.cursor()
    # Todo el comando SQL debe ir dentro de las comillas de execute()
    cursor.execute("SELECT nombre as 'Nombre', apellido as 'Apellido', email as 'Correo electronico' FROM estudiantes WHERE fecha_nacimiento > '2001-12-31'") 
    for r in cursor.fetchall():
        print(dict(r))
    conn.close()
...
def select_03():
    ...
    conn = conectar()
    cursor = conn.cursor()
    cursor.execute("SELECT nombre as 'Nombre', creditos as 'Creditos' FROM cursos ORDER BY creditos DESC")  # <-- completa aquí
    for r in cursor.fetchall():
        print(dict(r))
    conn.close()
...
def select_04():
    ...
    conn = conectar()
    cursor = conn.cursor()
    # Conectamos las tres tablas usando sus llaves foráneas (IDs relacionados)
    cursor.execute("""
        SELECT e.nombre AS estudiante, c.nombre AS curso, i.nota 
...
def insert_04():
    ...
    cursor.execute("""
        ...
        ('Valentina', 'Ruiz', 'valentina.ruiz@email.com', '2003-05-20'),
        ('Mateo', 'Torres', 'mateo.torres@email.com', '2002-11-14')
    """) # <-- completa aquí
    conn.commit()
...
# =========================================================
# EJERCICIOS - UPDATE (Modificar registros existentes)
# =========================================================
...
# =========================================================
# EJERCICIOS - DELETE (Borrar registros)
# =========================================================
